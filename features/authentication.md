...
## What it does
- **Register** (`POST /api/auth/register`) — email + password (min 8 chars), rejects duplicate emails (409). On success, hashes the password (PBKDF2-HMAC-SHA512, see [decisions/0002](../decisions/0002-pbkdf2-via-hashlib.md)), creates the user, and returns `{token, user}` (auto-login).
